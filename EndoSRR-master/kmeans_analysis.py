...
def main():
    ...
    imfiles = ['EndoRR_dataset/train/image/2_1_Left_Image.png']
    maskfiles = ['EndoRR_dataset/train/mask/2_1_Left_Image.png']
    
    # savepath = r"\\FS1\Docs4\annisa.sugiarti\My Documents\Codes\Microcamera_images\multiflash\kmeans_evaluation"
    savepath = '%s/kmeans_mask_dilation2_k10'%filepath
    ...
    for i in range(num):
        ...
        kmask_fl = kmask
        mask_fl = mask
        metric_WFM.step(pred=kmask_fl, gt=mask_fl)
        ...
        figs = []
        if i % 10 == 0:
            fig = plot_compare(img,kmask,mask)
            figname = os.path.join(savepath,os.path.splitext(os.path.split(imfiles[i])[-1])[0])
            # fig.savefig(figname)
            figs.append(fig)
    ...
    print('Weighted Fscore = %f'%wfm)
    # for fig in figs:
    #     fig.suptitle('dicescores = %f'%dicescores_avg)
    fig.suptitle('dicescores = %f'%dicescores_avg)

    kwargs['dicescores'] = dicescores_avg
    kwargs['weighted_f'] = wfm
